    ...
    while True:
        ...
        if value == True:
            ...
            if cache.get(state_name):
                ...
                number = 1
                for obj in obj_list:
                    obj = NationalSite(obj['name'],obj['category'],obj['address'],obj['zipcode'].strip(),'')
                    info = '['+str(number)+'] '+ obj.info()
                    number += 1
                    print(info)
            else:
                obj_list = get_sites_for_state(state_url)
                cache[state_name] = [obj.__dict__ for obj in get_sites_for_state(state_url)]
                number = 1
                for obj in obj_list:
                    info = '['+str(number)+'] '+obj.info()
                    obj = obj.__dict__
        ...
        try:
            ...
            name = obj.name
        print('----------------------------------')
        print('Places near '+name)
        print('----------------------------------')
        if cache[state_name][x].get('nearby'):
            print('Using cache')
            site_nearby = cache[state_name][x]['nearby']
            ...
            cache[state_name][x]['nearby'] = site_nearby
            for i in range(len(site_nearby['searchResults'])):
                name = site_nearby['searchResults'][i]['name']
                category = site_nearby['searchResults'][i]['fields']['group_sic_code_name']
                if category == '':
